...
namespace Wasit.Services.MapperConfig
{
    ...
    {
        public void MapConsumerDailyRentEstates(ICurrentUserService currentUser)
        {
            CreateMap<DailyRentEstate, ConsumerDailyRentEstateInfoDto>()
                ...
                .ForMember(dest => dest.IsAvailable, opt => opt.MapFrom(src => src.BookingFrom.HasValue && src.BookingTo.HasValue))
                .ForMember(dest => dest.IsFavourite, opt => opt.MapFrom(src => src.Favorites.Any(x => x.UserId == currentUser.UserId && x.DailyRentEstateId == src.Id)))
